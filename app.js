...
const mQuestionsAll = [
    ...
    [`How to rebaseline a repository:`, ``, ``, ``, ``, ``, `<p class='space'>git checkout --orphan temp_branch \\ create new branch with no history <p class='space'>git add -A  \\ add current files to branch <p class='space'>git commit -am "the first commit"  \\ commit files <p class='space'>git branch -D master  \\ delete master branch <p class='space'>git branch -m master  \\rename current branch master <p class='space'>git push -f origin master  \\ push update to online repository `, `IS445`]
